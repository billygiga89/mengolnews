...
namespace MengolNews.Api.Services
{
    ...
    public class VideosService
    {
        ...
        public async Task AquecerCacheAsync()
        {
            try
            {
                await GetVideosDoCanal(forcarAtualizacao: true);
                _logger.LogInformation("Cache de vídeos aquecido no startup.");
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Falha ao aquecer cache de vídeos no startup — seguirá tentando sob demanda.");
            }
        }
    }
}